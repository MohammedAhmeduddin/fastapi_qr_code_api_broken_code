...
from app.utils.common import encode_url_to_filename, generate_links
...
async def create_qr_code(request: QRCodeRequest, token: str = Depends(oauth2_scheme)):
    ...
    links = generate_links("create", qr_filename, SERVER_BASE_URL, qr_code_url)

    if qr_code_path.exists():
        return JSONResponse(status_code=status.HTTP_409_CONFLICT, content={"message": "QR code already exists.", "links": links})

    generate_qr_code(request.url, qr_code_path, FILL_COLOR, BACK_COLOR, request.size)
    return QRCodeResponse(message="QR code created successfully.", qr_code_url=qr_code_url, links=links)

@router.get("/", response_model=List[QRCodeResponse])
async def list_qr_codes(token: str = Depends(oauth2_scheme)):
